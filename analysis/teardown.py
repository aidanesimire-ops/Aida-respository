...
def main():
    ...
    for _, r in d.iterrows():
        land_ppsf = nbh.get(r["neighborhood"], {}).get("implied_land_ppsf")
        if not land_ppsf:
            continue
        ...
        if share >= 0.60 or (old and share >= 0.45):
            rows.append({
                ...
                "note": ("Land alone is ~{:.0f}% of the ask{}.".format(
                    share * 100, f"; {int(r['year_built'])} structure" if pd.notna(r.get("year_built")) else "")),
            })
